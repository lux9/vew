def louchebemize(sentence)
  ...
  words.each do |word|
    if word =~ /\W/ # \W stands for "any non-word character"
      ...
      if %w[a e i o u].include?(word[0])
        ...
        first_vowel_index = word.chars.index { |letter| %w[a e i o u].include?(letter) }
  ...
  result_elementselements.join # Build result sentence by joining tokens with space in-between.
end
